...

{
    ...
    if (!m_pdb.empty()) {
        try {
            ...
            m_stmt->execute("CREATE DATABASE if not exists celonis");
...
std::tuple<bool, std::string> KVDatabase::get(const std::string& key)
{
...

    {
        ...
        if (m_db.find(key) == m_db.end()) {
            ...
            while(m_res->next()) {
                ...
                EVA_LOG_DEBUG(g_logger) << "KVDatabase: get value for key " << key << " from persistent database" << ret;
                break;
            }
    ...
    put(key, ret); //invoke put, because it need to evict if the newly retrieved value go over the capacity
...
void KVDatabase::evict() {
    ...
    m_pstmt->execute();
    EVA_LOG_DEBUG(g_logger) << "KVDatabase: evict key: " << key << ", value: " << val;
    delete m_pstmt;
}
